...
export default function ChainOfThought(props: PromptTabProps) {
    ...
    const addMessageComponent = () => {
        if (messages.length > 0) {
            ...
            if (lastMessage.type === "USER") {
                setMessages([...messages, {type: "ANSWER", message: "", thoughts: null}]);
            } else {
    ...
    const handleSubmit = () => {
        ...
        if (lastMessage.thoughts?.some(thought => !thought.trim())) {
            alert("All thoughts are required.");
            return;
        }
    ...
    const addThought = (messageIndex: number) => {
        ...
        // Check if the target message's thoughts have reached the MAX_THOUGHTS_LIMIT
